...
ZIP = r"C:\Users\Milena\Desktop\dados\ECMWF"              # caminho do arquivo
ARQUIVO_EXTRAIDO = r"C:\Users\Milena\Desktop\dados\extrator\dados_extraidos"    # pasta onde os dat serão extraídos
SAIDA = r"C:\Users\Milena\Desktop\dados\planilhas\saidaECWMF.xlsx"                    # excel gerado
USINA = "PSATMAU"                                                           # usina
...
# Funcao que decide se o caminho de entrada é um .zip (extrai) ou já é uma pasta (usa direto):
def obter_pasta_dados(caminho_entrada: str, pasta_extracao: str) -> Path:
    ...
    if caminho.suffix.lower() == ".zip":
        return extrair_zip(caminho_entrada, pasta_extracao)

    raise ValueError(
        f"Caminho de entrada inválido: '{caminho_entrada}' não é uma pasta "
        f"nem um arquivo .zip"
    )
...
def processar_pasta(pasta: Path, palavra: str) -> list[list[str]]:
    ...
    for arquivo in arquivos_dat:
        # arquivos de série (padrão PREFIXO_m_DDMMAA, várias datas na mesma linha)
        if re.search(r"_m_\d{6}", arquivo.name):
...
def main():
    ...
    pasta = obter_pasta_dados(zip, ARQUIVO_EXTRAIDO)
